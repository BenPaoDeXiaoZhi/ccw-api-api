...
import { test, testAuthReadApi, testAuthWriteApi, expectKeys } from "src/testUtils";
...
test("get check in records should fail without channel", async () => {
  await testAuthReadApi(() => getCheckInRecords(), {
    ...
    validateShape: (res) => {
      ...
      assert.strictEqual(Array.isArray(res.checkInRecordResps), (true), "expected values to be strictly equal");
      assert.strictEqual(typeof res.checkInTimes, ("number"), "expected values to be strictly equal");
      assert.strictEqual(typeof res.todayIndex, ("number"), "expected values to be strictly equal");
    },
  });
});
