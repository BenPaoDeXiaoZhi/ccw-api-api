...
│   ├── web/
│   │   └── auth/
│   │       ├── login-by-password.ts
│   │       ├── logout.ts
│   │       └── logout_by_session.ts
...
- 文件名应准确描述 API 功能，如 `login-by-password.ts`、`create-short-url.ts`
...
// 7. 导出函数
export async function apiFunctionName(param1: string, param2: number): Promise<Res> {
 ...
 */
export async function loginByPassword(loginKey: string, password: string, reqExtra?: ReqExtra): Promise<Res<Extra>> {
  // ...
}
```
